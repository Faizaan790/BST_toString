...
*/
  ...
  public String toString() {
		...
		{
			t += (rectoString(root,i)).replace(" ", "")+"\n";
		}
		return t;
	}
	
	private String rectoString(BSTNode<T> node,int level) {
		...
		if (node == null)               
		  ...
		  else          
		  {
			  ...
			  {
				  return rectoString(node.getLeft(),level-1) + rectoString(node.getRight(),level-1);
			  }
